...
public record PatchModel(TextureAtlasSprite texture, boolean shaggify) implements BakedModel {
	...
	private BakedQuad quadFromVectors(Direction direction, float minX, float minY, float minZ, float maxX, float maxY, float maxZ) {
		BlockElementFace face = new BlockElementFace(null, 0, this.texture.atlasLocation().toString(), switch (direction) {
			case NORTH -> new BlockFaceUV(new float[]{maxX, minZ + 1.0F, minX, minZ}, 0);
			case EAST -> new BlockFaceUV(new float[]{maxX, minZ, maxX - 1.0F, maxZ}, 90);
			case SOUTH -> new BlockFaceUV(new float[]{minX, maxZ, maxX, maxZ - 1.0F}, 0);
			case WEST -> new BlockFaceUV(new float[]{minX, maxZ, minX + 1.0F, minZ}, 90);
			default -> new BlockFaceUV(new float[]{minX, minZ, maxX, maxZ}, 0);
		});

		return BAKERY.bakeQuad(new Vector3f(minX, minY, minZ), new Vector3f(maxX, maxY, maxZ), face, this.texture, direction, IDENTITY, null, true);
	}
