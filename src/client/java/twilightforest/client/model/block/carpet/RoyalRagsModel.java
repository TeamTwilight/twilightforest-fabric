...
public class RoyalRagsModel implements net.minecraft.client.resources.model.BakedModel, FabricBakedModel {
	@Nullable
	...
	private final Block[] validConnectors = {TFBlocks.CORONATION_CARPET.get()};

	public RoyalRagsModel(@Nullable List<BakedQuad>[] baseQuads, BakedQuad[][][] quads, TextureAtlasSprite particle, ItemOverrides overrides, ItemTransforms transforms) {
	...
	public void emitBlockQuads(BlockAndTintGetter level, BlockState state, BlockPos pos, Supplier<RandomSource> randomSupplier, RenderContext context) {
		...
		LoftyCarpetData data = this.buildData(level, pos);
	...
	public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, RandomSource random) {
		...
		return this.getQuads(side, new LoftyCarpetData());
	}

	private List<BakedQuad> getQuads(Direction side, LoftyCarpetData data) {
		ArrayList<BakedQuad> result = new ArrayList<>(4 + (this.baseQuads != null ? 4 : 0));
		if (side.getAxis().isHorizontal()) {
			if (this.baseQuads != null) {
				result.addAll(this.baseQuads[side.get2DDataValue()]);
			}
		} else {
	...
	private LoftyCarpetData buildData(BlockAndTintGetter getter, BlockPos pos) {
		LoftyCarpetData data = new LoftyCarpetData();
