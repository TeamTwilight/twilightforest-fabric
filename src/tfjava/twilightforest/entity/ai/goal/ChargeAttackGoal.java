...
/**
 * 1:1 port of upstream {@code twilightforest.entity.ai.goal.ChargeAttackGoal} —
 * line-charge attack used by Minotaur/Minoshroom and other {@link ITFCharger}s.
 *
 * <p>Codex Fabric port note: NeoForge {@code EventHooks.canEntityGrief} is replaced
 * with the equivalent vanilla check {@code level.getGameRules().getBoolean(
 * GameRules.RULE_MOBGRIEFING)} — same gameplay effect since codex doesn't fire
 * NF's pre-grief event hooks.</p>
 */
public class ChargeAttackGoal extends Goal {
	...
	public void tick() {
		...
		if (this.windup > 0) {
			...
			if (!this.charger.level().isClientSide() && this.charger.level().getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING)) {
