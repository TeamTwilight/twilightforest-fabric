...
/**
 * 1:1 port of upstream {@code twilightforest.entity.ai.goal.GroundAttackGoal} —
 * Minoshroom-specific charge-up + ground slam in a 15×3×15 box that pushes and
 * damages players on the ground.
 *
 * <p>Codex Fabric port note: upstream's {@code TFDamageTypes.getEntityDamageSource}
 * is renamed to {@code TFDamageTypes.entitySource} in codex; {@code TFSounds.X.get()}
 * → {@code TFSounds.X} since codex returns {@code SoundEvent} directly.</p>
 */
public class GroundAttackGoal extends Goal {
	...
	public void tick() {
		...
		// Codex Fabric: upstream sets {@code MoveControl.operation = WAIT} directly,
		// but the field/enum are package-private in vanilla 1.21.1. Stopping the
		// navigation has the equivalent freezing effect for the slam wind-up.
		...
		if (this.attackTick-- <= 0) {
			...
			for (Entity entity : hit) {
				if (entity.onGround()) {
					...
					entity.hurt(TFDamageTypes.entitySource(this.attacker.level(), TFDamageTypes.SLAM, this.attacker), (float) (this.attacker.getAttributeValue(Attributes.ATTACK_DAMAGE) * 0.5F));
				}
			}
		}
	}
