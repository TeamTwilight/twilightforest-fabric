...
/**
 * 1:1 port of upstream {@code twilightforest.entity.ai.goal.RedcapLightTNTGoal} —
 * locates a TNT block within 8 blocks, walks toward it with the flint-and-steel
 * visual swapped in, and ignites it on arrival.
 *
 * <p>Codex Fabric port note: NF {@code EventHooks.canEntityGrief} → vanilla
 * {@code GameRules.RULE_MOBGRIEFING}.</p>
 */
public class RedcapLightTNTGoal extends RedcapBaseGoal {
	...
	public void tick() {
		...
		if (this.redcap.distanceToSqr(Vec3.atLowerCornerOf(this.tntPos)) < 2.4D * 2.4D) {
			...
			// Codex Fabric: NF Block.onCaughtFire(...) was removed in vanilla 1.21.1.
			// Use TntBlock.explode() which spawns the PrimedTnt and removes the block
			// — same gameplay effect as upstream's manual onCaughtFire + air-set.
