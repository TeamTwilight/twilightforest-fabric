...
public class LichPopMobsGoal extends Goal {
	...
	public boolean canUse() {
		return !this.lich.isShadowClone()
				...
				&& !this.lich.level().getEntitiesOfClass(Mob.class, this.lich.getBoundingBox().inflate(32.0D, 16.0D, 32.0D),
						entity -> entity.getType().is(EntityTagGenerator.LICH_POPPABLES) && this.lich.hasLineOfSight(entity)).isEmpty();
	}
	...
	public void tick() {
		...
		super.tick();
		if (this.lich.getScepterTimeLeft() > 0 || !(this.lich.level() instanceof ServerLevel serverLevel)) {
			return;
		}
		for (Mob mob : this.lich.level().getEntitiesOfClass(Mob.class, this.lich.getBoundingBox().inflate(32.0D, 16.0D, 32.0D), entity -> entity.getType().is(EntityTagGenerator.LICH_POPPABLES) && entity != this.lich)) {
			if (this.lich.getSensing().hasLineOfSight(mob)) {
				mob.discard();
				LifedrainScepterItem.animateTargetShatter(serverLevel, mob);
