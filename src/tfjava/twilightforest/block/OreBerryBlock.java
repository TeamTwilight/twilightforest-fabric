...
/**
 * 1:1 port of upstream {@code twilightforest.block.OreBerryBlock} (NeoForge → Fabric:
...
public class OreBerryBlock extends TFBushBlock {
	...
	public void entityInside(BlockState state, Level level, BlockPos pos, Entity entity) {
		if (!(entity instanceof ItemEntity)) {
			entity.hurt(TFDamageTypes.source(level, TFDamageTypes.OREBERRY), 1.0F);
		}
	...
	// Codex Fabric port note:
	// Upstream NeoForge had `@Override Block#getBlockPathType` with PathType.DANGER_OTHER so mob
	// AI flagged ripe berries as dangerous-but-traversable. Vanilla Fabric 1.21.1 has no such hook
	// on Block (pathfinding goes through `isPathfindable(...)`). We keep the method intact (without
	// @Override) so any future Fabric mixin or content-registry-style integration can wire it back
	// in — and the 1:1 behaviour intent is preserved in source rather than discarded.
