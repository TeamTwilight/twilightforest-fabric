...
public record RechargeScepterEffect() implements EnchantmentEntityEffect {
    ...
    public static void applyRecharge(ServerLevel level, ItemStack item, Entity entity) {
        if (entity instanceof Player player && item.getDamageValue() == item.getMaxDamage()) {
            List<ScepterRepairRecipe> recipes = level.getRecipeManager().getAllRecipesFor(RecipeType.CRAFTING).stream()
                    .map(RecipeHolder::value)
                    .filter(ScepterRepairRecipe.class::isInstance)
                    .map(ScepterRepairRecipe.class::cast)
                    .toList();
                ...
                if (item.is(recipe.getScepter())) {
                    ...
                    if (slotsToConsume.size() == recipe.getRepairItems().size()) {
                        for (int slot : slotsToConsume) {
                            ...
                            Item craftingRemainder = stack.getItem().hasCraftingRemainingItem() ? stack.getItem().getCraftingRemainingItem() : null;
                            stack.shrink(1);
                            if (craftingRemainder != null) {
                                ItemStack remaining = new ItemStack(craftingRemainder);
