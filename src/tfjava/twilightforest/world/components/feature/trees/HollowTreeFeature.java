...
//						putBlockAndMetadata(dx + x, dy + y, dz + z, Blocks.LADDER,  4, true);
	...
	public static void addBug(LevelAccessor level, Block bug, BlockPos pos, int diameter, int fHeight, double fAngle) {
		...
		BlockState bugState = TFBlockStateHelper.setFacingIfPossible(bug.defaultBlockState(), facing);
		if (bugState.canSurvive(level, src)) {
			level.setBlock(src, bugState, Block.UPDATE_ALL);
		}
	}
}
