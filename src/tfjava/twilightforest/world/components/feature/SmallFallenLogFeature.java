...
public class SmallFallenLogFeature extends Feature<HollowLogConfig> {
	...
	public boolean place(FeaturePlaceContext<HollowLogConfig> ctx) {
		...
		if (goingX) {
			...
			if (hollowLogState != null) {
				hollowLogState = setAxisIfPossible(hollowLogState, Direction.Axis.X);
				hollowLogState = setVariantIfPossible(hollowLogState, determineHollowProperties(world, pos, rand));
			}
			...
			if (hollowLogState != null) {
				hollowLogState = setAxisIfPossible(hollowLogState, Direction.Axis.Z);
				hollowLogState = setVariantIfPossible(hollowLogState, determineHollowProperties(world, pos, rand));
			}
	...
	// Phase F1.10 — guards for hollow log states whose runtime block (a
	// CodexRotatedPillarBlock) holds RotatedPillarBlock.AXIS [x,y,z]
	// instead of HorizontalHollowLogBlock's HORIZONTAL_AXIS [x,z] / VARIANT.
	// Crash root cause: setValue(HORIZONTAL_AXIS,...) on a state lacking that
	// property throws IAE and aborts chunk feature application, blocking
	// chunk load entirely. Same mitigation pattern as F1.9 setFacingIfPossible.
	private static BlockState setAxisIfPossible(BlockState state, Direction.Axis axis) {
		if (state.hasProperty(HorizontalHollowLogBlock.HORIZONTAL_AXIS))
			return state.setValue(HorizontalHollowLogBlock.HORIZONTAL_AXIS, axis);
		if (state.hasProperty(RotatedPillarBlock.AXIS))
			return state.setValue(RotatedPillarBlock.AXIS, axis);
		return state;
	}

	private static BlockState setVariantIfPossible(BlockState state, HollowLogVariants.Horizontal variant) {
		return state.hasProperty(HorizontalHollowLogBlock.VARIANT)
				? state.setValue(HorizontalHollowLogBlock.VARIANT, variant)
				: state;
	}
