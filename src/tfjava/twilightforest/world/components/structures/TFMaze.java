...
public class TFMaze {
	...
	private void putCanopyTree(WorldGenLevel world, ChunkGenerator generator, int x, int y, int z, TFStructureComponentOld component, BoundingBox sbb) {
		...
		if (sbb.isInside(pos)) {
			ConfiguredFeature<?, ?> canopyTree = world.registryAccess().registryOrThrow(Registries.CONFIGURED_FEATURE).get(TFConfiguredFeatures.CANOPY_TREE);
			if (canopyTree == null || !canopyTree.place(world, generator, world.getRandom(), pos)) {
