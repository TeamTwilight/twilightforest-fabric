		...
		if (raytraceresult.getType() == HitResult.Type.MISS) {
			...
			if (raytraceresult.getType() == HitResult.Type.BLOCK) {
				...
				if ((ifluidstate.getType() == Fluids.WATER || blockstate.is(BlockTags.ICE)) && level.isEmptyBlock(blockpos1)
					...
					final BlockState lilypad = getBlock().defaultBlockState();
					level.setBlock(blockpos1, lilypad, Block.UPDATE_ALL_IMMEDIATE);
					level.setBlock(blockpos1.east(), lilypad, Block.UPDATE_ALL_IMMEDIATE);
					level.setBlock(blockpos1.east().south(), lilypad, Block.UPDATE_ALL_IMMEDIATE);
					level.setBlock(blockpos1.south(), lilypad, Block.UPDATE_ALL_IMMEDIATE);
