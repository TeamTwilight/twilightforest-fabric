...
public class ChainBlockItem extends Item {
	...
	public boolean isCorrectToolForDrops(ItemStack stack, BlockState state) {
		...
		for (it.unimi.dsi.fastutil.objects.Object2IntMap.Entry<net.minecraft.core.Holder<net.minecraft.world.item.enchantment.Enchantment>> entry : stack.getEnchantments().entrySet()) {
			if (entry.getKey().is(TFEnchantments.DESTRUCTION)) {
				return this.getHarvestLevel(entry.getIntValue()).createToolProperties(BlockTagGenerator.MINEABLE_WITH_BLOCK_AND_CHAIN).isCorrectForDrops(state);
			}
		}
		return false;
	}
