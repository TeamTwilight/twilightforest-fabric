...
public class CasketRepairExtension implements ICraftingCategoryExtension<CasketRepairRecipe> {
	...
	public void setRecipe(RecipeHolder<CasketRepairRecipe> recipeHolder, IRecipeLayoutBuilder builder, ICraftingGridHelper craftingGridHelper, IFocusGroup focuses) {
		var casket = new ItemStack(TFItems.KEEPSAKE_CASKET.get().builtInRegistryHolder(), 1, DataComponentPatch.builder().set(TFDataComponents.CASKET_DAMAGE, 2).build());
		List<List<ItemStack>> inputs = new ArrayList<>();
		inputs.add(List.of(casket));
		inputs.add(List.of(new ItemStack(TFItems.CHARM_OF_KEEPING_3.get())));

		craftingGridHelper.createAndSetInputs(builder, inputs, 0, 0);
		builder.setShapeless();

		var repairedScepter = new ItemStack(TFItems.KEEPSAKE_CASKET.get().builtInRegistryHolder(), 1, DataComponentPatch.builder().set(TFDataComponents.CASKET_DAMAGE, 1).build());
