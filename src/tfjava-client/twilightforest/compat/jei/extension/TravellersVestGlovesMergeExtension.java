...
public class TravellersVestGlovesMergeExtension implements ICraftingCategoryExtension<TravellersVestGlovesMergeRecipe> {
	...
	public void setRecipe(RecipeHolder<TravellersVestGlovesMergeRecipe> recipeHolder, IRecipeLayoutBuilder builder, ICraftingGridHelper craftingGridHelper, IFocusGroup focuses) {
		craftingGridHelper.createAndSetInputs(builder, new ArrayList<>(List.of(List.of(new ItemStack(TFItems.TRAVELLERS_VEST.get())), List.of(new ItemStack(TFItems.TRAVELLERS_GLOVES.get())))), 0, 0);
		builder.setShapeless();
		craftingGridHelper.createAndSetOutputs(builder, List.of(new ItemStack(TFItems.TRAVELLERS_VEST.get().builtInRegistryHolder(), 1, DataComponentPatch.builder().set(TFDataComponents.TRAVELLERS_HAS_GLOVES, Unit.INSTANCE).build())));
	}
}
