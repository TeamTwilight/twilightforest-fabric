...
public abstract class TFPartRenderer<T extends TFPart<?>, M extends ListModel<T>> extends EntityRenderer<T> {
    ...
    protected TFPartRenderer(EntityRendererProvider.Context context, M model) {
        super(context);
        this.model = model;
    }
    ...
    public int getOverlayCoords(T entity, float u) {
        if (entity.getParent() instanceof LivingEntity living) {
            return OverlayTexture.pack(
                    OverlayTexture.u(u),
                    OverlayTexture.v(living.hurtTime > 0 || living.deathTime > 0 || entity.hurtTime > 0 || entity.deathTime > 0));
        }
