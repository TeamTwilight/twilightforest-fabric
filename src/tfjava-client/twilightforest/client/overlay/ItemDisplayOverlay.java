...
public class ItemDisplayOverlay {
	...
	private static void renderHolders(GuiGraphics graphics, Minecraft minecraft, Gui gui, Player player, List<DisplayHolder> typesToRender, int widest) {
		...
		graphics.pose().translate(TFConfig.itemDisplayScreenOffsetX, TFConfig.itemDisplayScreenOffsetY, 0.0D);
		graphics.pose().scale((float) TFConfig.itemDisplayScreenScale, (float) TFConfig.itemDisplayScreenScale, (float) TFConfig.itemDisplayScreenScale);
	...
	private static int fillDisplayHolders(List<DisplayHolder> typesToRender, ItemDisplayContents contents, Minecraft minecraft, Gui gui, Player player) {
		...
		for (int i = 0; i < slots; i++) {
			...
			ItemDisplay display = displayFor(type);
			if (display == null)
				continue;
	...
	private static ItemDisplay displayFor(ItemDisplayType type) {
		if (type == ItemDisplays.MAP) return new MapDisplay();
		if (type == ItemDisplays.COMPASS) return new CompassDisplay();
		if (type == ItemDisplays.CLOCK) return new ClockDisplay();
		if (type == ItemDisplays.MOON_DIAL) return new MoonDialDisplay();
		return null;
	}
