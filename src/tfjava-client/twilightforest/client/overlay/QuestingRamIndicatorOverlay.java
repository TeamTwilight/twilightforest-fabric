...
public class QuestingRamIndicatorOverlay {
	...
	public static void render(Minecraft minecraft, GuiGraphics graphics, Gui gui, Player player) {
		if (player != null && !minecraft.options.hideGui && TFConfig.showQuestRamCrosshairIndicator) {
			if (minecraft.gameMode != null && minecraft.options.getCameraType().isFirstPerson() && (minecraft.gameMode.getPlayerMode() != GameType.SPECTATOR || gui.canRenderCrosshairForSpectator(minecraft.hitResult)) && minecraft.crosshairPickEntity instanceof QuestRam ram) {
				ItemStack stack = player.getInventory().getItem(player.getInventory().selected);
				if (!stack.isEmpty()) {
					for (var questEntry : QuestReloadListener.currentContext().getContext().questItems().entrySet()) {
