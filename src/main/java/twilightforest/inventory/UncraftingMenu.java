...
public class UncraftingMenu extends RecipeBookMenu {
	...
	public void slotsChanged(Container inventory) {
		...
		if (inventory == this.tinkerInput) {
			...
			if (size > 0 && !inputStack.is(TFItemTags.BANNED_UNCRAFTABLES)) {
				...
				this.uncraftingMatrix.numberOfInputItems = recipe instanceof UncraftingRecipe uncraftingRecipe ? uncraftingRecipe.getCount() : ((Recipe<CraftingInput>) recipe).assemble(CraftingInput.EMPTY).getCount(); //Uncrafting recipes need this method call
	...
	public static boolean isIngredientProblematic(ItemStack ingredient) {
		return (!ingredient.isEmpty() && !ingredient.getItem().getCraftingRemainder().item().equals(Items.AIR)) || ingredient.is(Items.BARRIER);
	}
		...
		if (!inputStack.isEmpty()) {
			for (RecipeHolder<?> recipe : world.getServer().getRecipeManager().getRecipes()) {
				if (isRecipeSupported(recipe.value()) &&
					!false &&
					!ingredientsOf(recipe.value()).isEmpty() &&
					matches(inputStack, ((Recipe<CraftingInput>) recipe.value()).assemble(CraftingInput.EMPTY)) &&
			...
			for (RecipeHolder<UncraftingRecipe> uncraftingRecipe : world.getServer().getRecipeManager().getRecipes().stream().filter(r -> r.value() instanceof UncraftingRecipe).map(r -> (RecipeHolder<UncraftingRecipe>) r).toList()) {
	...
	private static RecipeHolder<CraftingRecipe>[] getRecipesFor(CraftingInput input, Level level) {
		return level.getServer().getRecipeManager().getRecipes().stream()
			.filter(r -> r.value() instanceof CraftingRecipe cr && cr.matches(input, level))
	...
	private void chooseRecipe(CraftingInput input) {
		...
		if (recipe != null && ((ServerPlayer) this.player).getRecipeBook().contains(recipe.id())) {
	...
	@Override
	public void clicked(int slotNum, int mouseButton, net.minecraft.world.inventory.ContainerInput clickType, Player player) {
		...
		if (slotNum > 0 && this.getSlotContainer(slotNum) == this.assemblyMatrix
			...
			if (this.assemblyMatrix.isEmpty() && (clickType != net.minecraft.world.inventory.ContainerInput.SWAP || player.getInventory().getItem(mouseButton).isEmpty())) {
				slotNum -= 9;
			}
		}
	...
	private ItemStack[] getIngredients(Recipe<?> recipe) {
		...
		for (int i = 0; i < ingredients.size(); i++) {
			ItemStack[] matchingStacks = ingredients.get(i).items().map(ItemStack::new).filter(s -> !s.is(TFItemTags.BANNED_UNCRAFTING_INGREDIENTS)).toArray(ItemStack[]::new);
	...
	private static List<net.minecraft.world.item.crafting.Ingredient> ingredientsOf(Recipe<?> recipe) {
		return switch (recipe) {
			case net.minecraft.world.item.crafting.ShapedRecipe s -> s.getIngredients().stream().flatMap(java.util.Optional::stream).toList();
			default -> List.of();
		};
	}
	...
	@Override
	public net.minecraft.world.inventory.RecipeBookMenu.PostPlaceAction handlePlacement(boolean placeAll, boolean wipeActive, RecipeHolder<?> recipe, net.minecraft.server.level.ServerLevel level, Inventory inv) {
		new UncrafterPlaceRecipe<>(this).recipeClicked(inv.player, (RecipeHolder<Recipe<RecipeInput>>) recipe, placeAll);
		return net.minecraft.world.inventory.RecipeBookMenu.PostPlaceAction.NOTHING;
	}
}
