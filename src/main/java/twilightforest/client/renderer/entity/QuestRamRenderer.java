...
public class QuestRamRenderer extends MobRenderer<QuestRam, QuestingRamRenderState, QuestRamModel> {
	...
	public static final Identifier TEXTURE = TFCommon.getModelTexture("questram.png");
	public static final Identifier LINE_TEXTURE = TFCommon.getModelTexture("questram_lines.png");
	...
	public static class GlowingLinesLayer extends RenderLayer<QuestingRamRenderState, QuestRamModel> {
		...
		public void submit(PoseStack stack, SubmitNodeCollector collector, int light, QuestingRamRenderState state, float yRot, float xRot) {
			...
			collector.order(1).submitModel(this.getParentModel(), state, stack, RenderTypes.entityTranslucent(LINE_TEXTURE), LightCoordsUtil.FULL_BRIGHT, OverlayTexture.NO_OVERLAY, state.outlineColor, null);
		}
	}
}
