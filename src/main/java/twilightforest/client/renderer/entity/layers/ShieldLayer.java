...
public class ShieldLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
	...
	public void render(PoseStack stack, MultiBufferSource buffer, int light, T entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		if (this.getShieldCount(entity) > 0) {
			this.renderShields(stack, buffer, light, entity, partialTicks);
		}
	}
	...
	private int getShieldCount(T entity) {
		return entity instanceof Lich lich
			...
			: entity.getAttachedOrCreate(TFDataAttachments.FORTIFICATION_SHIELDS).shieldsLeft();
	}

	private void renderShields(PoseStack stack, MultiBufferSource buffer, int light, T entity, float partialTicks) {
		...
		BakedModel model = Minecraft.getInstance().getModelManager().getModel(SHIELD_MODEL);

		for (int c = 0; c < count; c++) {
			stack.pushPose();

			// shift to the center of the entity, invert Y to fix rendering direction
			...
			for (Direction dir : DIRS) {
				for (BakedQuad quad : model.getQuads(null, dir, entity.getRandom())) {
					consumer.putBulkData(pose, quad, 1.0F, 1.0F, 1.0F, 1.0F, light, OverlayTexture.NO_OVERLAY);
				}
			}
