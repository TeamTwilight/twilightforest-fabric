...
public class TFChestRenderer<T extends ChestBlockEntity> extends ChestRenderer<T> {
	...
	private static EnumMap<ChestType, SpriteId> chestMaterial(String type, String suffix) {
		...
		map.put(ChestType.SINGLE, Sheets.CHEST_MAPPER.apply(TFCommon.prefix(type + "/" + suffix)));
		map.put(ChestType.LEFT, Sheets.CHEST_MAPPER.apply(TFCommon.prefix(type + "/" + suffix + "_left")));
		map.put(ChestType.RIGHT, Sheets.CHEST_MAPPER.apply(TFCommon.prefix(type + "/" + suffix + "_right")));
		return map;
	}
}
