...
public class CicadaRenderer implements BlockEntityRenderer<CicadaBlockEntity, CicadaRenderState> {
	...
	public void submit(CicadaRenderState state, PoseStack stack, SubmitNodeCollector collector, CameraRenderState camera) {
		renderCicada(this.cicadaModel, state.yaw, state.rotation, state.facing, stack, collector, state.lightCoords, state.breakProgress);
	}

	public static void renderCicada(CicadaModel model, float yaw, float rotation, Direction facing, PoseStack stack, SubmitNodeCollector collector, int light, ModelFeatureRenderer.@Nullable CrumblingOverlay overlay) {
		stack.pushPose();
		...
		collector.submitModel(model, Unit.INSTANCE, stack, model.renderType(TEXTURE), light, OverlayTexture.NO_OVERLAY, 0, overlay);
		stack.popPose();
	}
}
