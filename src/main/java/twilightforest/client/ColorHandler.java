...
public final class ColorHandler {
	...
	public static void registerBlockColors() {
		...
		ColorProviderRegistry.BLOCK.register((state, getter, pos, tintIndex) -> tintIndex > 15 ? 0xFFFFFF : ColorUtil.hsvToRGB(getter == null ? 0.45F : AuroraBrickBlock.rippleFractialNoise(2, 128.0f, pos != null ? pos.above(128) : new BlockPos(0, 0, 0), 0.37f, 0.67f, 1.5f), 1.0f, 1.0f), TFBlocks.AURORA_BLOCK.get());
		ColorProviderRegistry.BLOCK.register((state, getter, pos, tintIndex) -> {
			if (tintIndex > 15) return 0xFFFFFF;

			int normalColor = ColorProviderRegistry.BLOCK.get(TFBlocks.AURORA_BLOCK.get()).getColor(TFBlocks.AURORA_BLOCK.get().defaultBlockState(), getter, pos, tintIndex);
		...
		ColorProviderRegistry.BLOCK.register((state, getter, pos, tintIndex) -> tintIndex > 15 ? 0xFFFFFF : ColorProviderRegistry.BLOCK.get(Blocks.GRASS).getColor(Blocks.GRASS.defaultBlockState(), getter, pos, tintIndex), TFBlocks.SMOKER.get(), TFBlocks.FIRE_JET.get());
		ColorProviderRegistry.BLOCK.register((state, getter, pos, tintIndex) -> getter != null && pos != null ? 2129968 : 7455580, TFBlocks.HUGE_LILY_PAD.get());
		ColorProviderRegistry.BLOCK.register((state, getter, pos, tintIndex) -> {
			if (tintIndex > 15) return 0xFFFFFF;
		...
		ColorProviderRegistry.BLOCK.register((state, getter, pos, tintIndex) -> GrassColor.get(0.5D, 1.0D),
