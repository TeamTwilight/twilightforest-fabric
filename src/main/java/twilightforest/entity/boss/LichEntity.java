...
public class LichEntity extends Monster {
	...
	private static final EntityDataAccessor<Boolean> DATA_ISCLONE = SynchedEntityData.defineId(LichEntity.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Byte> DATA_SHIELDSTRENGTH = SynchedEntityData.defineId(LichEntity.class, EntityDataSerializers.BYTE);
	private static final EntityDataAccessor<Byte> DATA_MINIONSLEFT = SynchedEntityData.defineId(LichEntity.class, EntityDataSerializers.BYTE);
	private static final EntityDataAccessor<Byte> DATA_ATTACKTYPE = SynchedEntityData.defineId(LichEntity.class, EntityDataSerializers.BYTE);
	...
	public LichEntity(EntityType<? extends LichEntity> type, Level world) {
		super(type, world);
	...
	public LichEntity(Level world, LichEntity otherLich) {
	...
	public boolean hurt(DamageSource src, float damage) {
		...
		if (src.getEntity() instanceof LichEntity) {
			return false;
		}
		...
		if (super.hurt(src, damage)) {
			...
			if (getLastHurtByMob() instanceof LichEntity && ((LichEntity) getLastHurtByMob()).masterLich == this.masterLich) {
...
//					world.playSoundAtEntity(mob, mob.getDeathSound(), mob.getSoundVolume(), (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
	...
	public int countMyMinions() {
		return (int) level.getEntitiesOfClass(LichMinionEntity.class, new AABB(getX(), getY(), getZ(), getX() + 1, getY() + 1, getZ() + 1).inflate(32.0D, 16.0D, 32.0D))
				.stream()
