...
public class TomeBolt extends TFThrowable implements ItemSupplier {
	...
	public TomeBolt(EntityType<? extends TomeBolt> type, Level world, LivingEntity thrower) {
		super(type, world, thrower, new ItemStack(Items.PAPER));
	}
	...
	public void handleEntityEvent(byte id) {
		if (id == EntityEvent.DEATH) {
			ParticleOptions particle = new ItemParticleOption(ParticleTypes.ITEM, new ItemStackTemplate(Items.PAPER));
			for (int i = 0; i < 8; ++i) {
	...
	protected void onHitEntity(EntityHitResult result) {
		...
		if (this.level() instanceof ServerLevel server && result.getEntity() instanceof LivingEntity living) {
			if (result.getEntity().hurtServer(server, TFDamageTypes.getIndirectEntityDamageSource(this.level(), this.random.nextBoolean() ? TFDamageTypes.LOST_WORDS : TFDamageTypes.SCHOOLED, this, this.getOwner()), 3)) {
