...
public class CarminiteGolem extends Monster {
    ...
    public CarminiteGolem(EntityType<? extends CarminiteGolem> type, Level level) {
        super(type, level);
    }
    ...
    public void aiStep() {
        ...
        if (this.getRandom().nextBoolean()) {
            this.level().addParticle(new DustParticleOptions(new Vector3f(1.0F, 0.0F, 0.0F), 1.0F), this.getX() + (this.getRandom().nextDouble() - 0.5D) * this.getBbWidth(), this.getY() + this.getRandom().nextDouble() * this.getBbHeight() - 0.25D, this.getZ() + (this.getRandom().nextDouble() - 0.5D) * this.getBbWidth(), 0.0D, 0.0D, 0.0D);
        }
    }
