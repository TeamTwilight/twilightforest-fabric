...
public class StableIceCore extends BaseIceMob implements RangedAttackMob {
    public StableIceCore(EntityType<? extends StableIceCore> type, Level level) {
        super(type, level);
    }
    ...
    public void performRangedAttack(LivingEntity target, float distanceFactor) {
        IceSnowball snowball = new IceSnowball(TFEntities.ICE_SNOWBALL.get(), this.level(), this);
        snowball.setPos(this.getX(), this.getY() + this.getBbHeight() * 0.6F, this.getZ());
        double targetY = target.getY() + target.getEyeHeight() - 1.4D;
        double deltaX = target.getX() - this.getX();
        double deltaY = targetY - snowball.getY();
        double deltaZ = target.getZ() - this.getZ();
        float arc = Mth.sqrt((float) (deltaX * deltaX + deltaZ * deltaZ)) * 0.2F;
        snowball.shoot(deltaX, deltaY + arc, deltaZ, 1.6F, 6.0F);
