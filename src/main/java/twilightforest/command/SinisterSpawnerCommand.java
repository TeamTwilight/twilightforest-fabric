...
public final class SinisterSpawnerCommand {
    private static final SuggestionProvider<CommandSourceStack> SUGGEST_LOOT_TABLE = (context, builder) -> {
    ...
    public LiteralArgumentBuilder<CommandSourceStack> register(CommandBuildContext buildContext) {
        return Commands.literal("sinister_spawner")
            ...
            .then(Commands.literal("add_particle")
                .then(Commands.argument("particle", ParticleArgument.particle(buildContext))
                    .then(Commands.argument("pos", BlockPosArgument.blockPos()).executes(this::addParticle))))
            .then(Commands.literal("remove_particle")
                .then(Commands.argument("particle", ParticleArgument.particle(buildContext))
                    .then(Commands.argument("pos", BlockPosArgument.blockPos()).executes(this::removeParticle))))
            .then(Commands.literal("set_loot")
                .then(Commands.argument("loot", ResourceOrIdArgument.lootTable(buildContext)).suggests(SUGGEST_LOOT_TABLE)
                    .then(Commands.argument("pos", BlockPosArgument.blockPos()).executes(this::setLootTable))))
            .then(Commands.literal("clear_loot")
                .then(Commands.argument("pos", BlockPosArgument.blockPos()).executes(this::clearLootTable)));
    }
    ...
    private int addParticle(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (context.getSource().getLevel().getBlockEntity(pos) instanceof SinisterSpawnerBlockEntity blockEntity
            && blockEntity.addParticle(options, true)) {
            return 1;
        }
        return 0;
    }
    ...
    private int removeParticle(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        BlockPos pos = BlockPosArgument.getLoadedBlockPos(context, "pos");
        if (context.getSource().getLevel().getBlockEntity(pos) instanceof SinisterSpawnerBlockEntity blockEntity
            && blockEntity.removeParticle(options, true)) {
            return 1;
        }
        return 0;
    }
    ...
    private int setLootTable(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        ResourceKey<LootTable> lootKey = loot.unwrapKey().orElse(null);
        if (context.getSource().getLevel().getBlockEntity(pos) instanceof SinisterSpawnerBlockEntity blockEntity
            && blockEntity.setLootTable(lootKey)) {
            return 1;
        }
        return 0;
    }
