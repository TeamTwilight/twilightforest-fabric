...
public class ShieldCommand {
    ...
    private static int add(Entity e, int num, boolean temporary) {
        CapabilityList.SHIELD_CAPABILITY_COMPONENT_KEY.maybeGet(e).ifPresent(cap -> cap.addShields(num, temporary));
        return Command.SINGLE_SUCCESS;
    }

    private static int set(Entity e, int num, boolean temporary) {
        CapabilityList.SHIELD_CAPABILITY_COMPONENT_KEY.maybeGet(e).ifPresent(cap -> cap.setShields(num, temporary));
        return Command.SINGLE_SUCCESS;
    }
}
