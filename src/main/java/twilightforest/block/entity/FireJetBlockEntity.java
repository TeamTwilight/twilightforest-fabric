...
public class FireJetBlockEntity extends BlockEntity {
	...
	private void tickPopping(Level level, BlockPos pos, BlockState state, FireJetBlockEntity te) {
		if (++te.counter >= 80) {
			...
			if (te.counter % 20 == 0) {
				...
				level.playSound(null, pos, TFSounds.JET_POP.value(), SoundSource.BLOCKS, 0.2F + level.getRandom().nextFloat() * 0.2F, 0.9F + level.getRandom().nextFloat() * 0.15F);
			}
		}
	}
	...
	private void tickFlame(Level level, BlockPos pos, BlockState state, FireJetBlockEntity te) {
		...
		if (level.isClientSide()) {
			...
			// sounds
			if (te.counter % 4 == 0) {
				level.playLocalSound(x + 0.5, y + 0.5, z + 0.5, TFSounds.JET_ACTIVE.value(), SoundSource.BLOCKS, 1.0F + level.getRandom().nextFloat(), level.getRandom().nextFloat() * 0.7F + 0.3F, false);

			} else if (te.counter == 1) {
				level.playLocalSound(x + 0.5, y + 0.5, z + 0.5, TFSounds.JET_START.value(), SoundSource.BLOCKS, 1.0F + level.getRandom().nextFloat(), level.getRandom().nextFloat() * 0.7F + 0.3F, false);
			}
		}
