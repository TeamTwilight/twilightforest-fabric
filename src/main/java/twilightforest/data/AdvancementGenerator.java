...
public class AdvancementGenerator extends FabricAdvancementProvider {
	...
	public void generateAdvancement(HolderLookup.Provider registryLookup, Consumer<AdvancementHolder> consumer) {
		...
		HolderLookup.RegistryLookup<Structure> structures = registryLookup.lookupOrThrow(Registries.STRUCTURE);
...
//				.save(consumer, "twilightforest:progress_castle");
		...
		Advancement.Builder.advancement().parent(e115).display(
				...
				null, AdvancementType.CHALLENGE, true, true, true)
			.addCriterion("eat_115_e115", PlayerTrigger.TriggerInstance.located(Optional.of(EntityPredicate.Builder.entity().subPredicate(PlayerPredicate.Builder.player().addStat(Stats.CUSTOM, registryLookup.lookupOrThrow(Registries.CUSTOM_STAT).getOrThrow(TFStats.E115_SLICES_EATEN.getKey()), MinMaxBounds.Ints.atLeast(115)).build()).build())))
	...
	private ItemStack e115Tag(String key) {
		...
		itemstack.set(TFDataComponents.EXPERIMENT_115_VARIANTS.get(), key);
		return itemstack;
	}

	private ItemStack flaskWithHarming() {
		ItemStack itemstack = new ItemStack(TFItems.BRITTLE_FLASK.get());
		itemstack.set(TFDataComponents.POTION_FLASK_CONTENTS.get(), new PotionFlaskComponent(new PotionContents(Potions.STRONG_HARMING), 4, 0, false));
		return itemstack;
	}
