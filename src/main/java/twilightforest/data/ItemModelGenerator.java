...
//		return generated(Registry.ITEM.getKey(item).getPath(), prefix("items/" + Registry.ITEM.getKey(item).getPath()));
//	}
//
//	private ItemModelBuilder biggerTex(Item item, ResourceLocation... layers) {
//		ItemModelBuilder builder = withExistingParent(Registry.ITEM.getKey(item).getPath(), "twilightforest:item/util/overlap_gui");
//		for (int i = 0; i < layers.length; i++) {
//			builder = builder.texture("layer" + i, layers[i]);
//		}
//		return builder;
