...
public class NoReturnTeleporter extends TFTeleporter {
	...
	private static Vec3 placePosition(Entity entity, ServerLevel level, Vec3 pos) {
		...
		if (spot != null) {
			TFCommon.LOGGER.debug("Found existing teleportation for {} at {}", name, spot);
		...
		if (spot != null) {
			TFCommon.LOGGER.debug("Found ideal teleportation spot for {} at {}", name, spot);
			return safePosInColumn(level, entity, Vec3.atCenterOf(spot.above()));
		}

		TFCommon.LOGGER.debug("Did not find ideal teleportation spot, shooting for okay one for {}", name);
		spot = findPortalCoords(level, pos, blockPos -> isOkayForPortal(level, blockPos));

		if (spot != null) {
			TFCommon.LOGGER.debug("Found okay teleportation spot for {} at {}", name, spot);
			return safePosInColumn(level, entity, Vec3.atCenterOf(spot.above()));
		}

		// well I don't think we can actually just return and fail here
		TFCommon.LOGGER.debug("Did not even find an okay teleportation spot, just making a random one for {}", name);
