...
public class AvoidLandmarkGridPlacement extends RandomSpreadStructurePlacement {
    public static final MapCodec<AvoidLandmarkGridPlacement> CODEC = RecordCodecBuilder.<AvoidLandmarkGridPlacement>mapCodec(instance -> instance.group(
        ...
        StructurePlacement.FrequencyReductionMethod.CODEC.optionalFieldOf("frequency_reduction_method", StructurePlacement.FrequencyReductionMethod.DEFAULT).forGetter(AvoidLandmarkGridPlacement::frequencyReductionMethod),
    ...
    protected boolean isPlacementChunk(ChunkGeneratorStructureState state, int chunkX, int chunkZ) {
        ...
        if (potentialChunk.x != chunkX || potentialChunk.z != chunkZ) {
            return false;
        }

        BlockPos.MutableBlockPos featurePos = LegacyLandmarkPlacements.getNearestCenterXZ(chunkX, chunkZ).mutable();
        featurePos.set(Math.abs(featurePos.getX() - potentialChunk.getWorldPosition().getX()), 0, Math.abs(featurePos.getZ() - potentialChunk.getWorldPosition().getZ()));
        int size = 80;
    ...
    public record AvoidAdditionalStructures(Object2IntArrayMap<Holder<StructureSet>> avoidStructures) {
        ...
        private boolean isPlacementForbidden(ChunkGeneratorStructureState structureState, int x, int z) {
            for (Object2IntArrayMap.Entry<Holder<StructureSet>> entry : this.avoidStructures.object2IntEntrySet()) {
                if (structureState.hasStructureChunkInRange(entry.getKey(), x, z, entry.getIntValue())) {
                    return true;
                }
            }
            return false;
