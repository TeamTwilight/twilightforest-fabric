...
public final class LichTowerFoyer extends TwilightJigsawPiece implements IPieceBeardifierModifier, SpawnIndexProvider {
	...
	public LichTowerFoyer(StructureTemplateManager structureManager, JigsawPlaceContext placeContext, boolean putChest, boolean chestSide) {
		super(TFStructurePieceTypes.LICH_TOWER_FOYER, 0, structureManager, TFCommon.prefix("lich_tower/tower_foyer"), placeContext);
	...
	protected void processJigsaw(TwilightJigsawPiece parent, StructurePieceAccessor pieceAccessor, Structure.GenerationContext context, JigsawRecord connection, int jigsawIndex) {
		if ("twilightforest:lich_tower/tower_base".equals(connection.target())) {
			JigsawPlaceContext placeableJunction = JigsawPlaceContext.pickPlaceableJunction(this.templatePosition(), connection.pos(), connection.orientation(), this.structureManager, TFCommon.prefix("lich_tower/tower_base"), "twilightforest:lich_tower/tower_base", context.random());
			...
			JigsawPlaceContext placeableJunction = JigsawPlaceContext.pickPlaceableJunction(this.templatePosition(), connection.pos(), connection.orientation(), this.structureManager, TFCommon.prefix("lich_tower/foyer_decor"), "twilightforest:shelf", context.random());
