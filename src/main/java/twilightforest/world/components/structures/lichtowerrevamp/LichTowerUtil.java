...
public final class LichTowerUtil {
	...
	static BlockPos getRandomOpeningPlacementPos(BlockPos origin, Direction side, Mirror mirror, RandomSource random, int segmentLevel, boolean sidesFaceXAxis) {
		...
		return switch (side) {
			case EAST ->
					origin.offset(-1, pair.z, mirror == Mirror.FRONT_BACK ? CentralTowerSegment.SIDE_LENGTH - pair.x : pair.x);
			case WEST ->
					origin.offset(CentralTowerSegment.SIDE_LENGTH, pair.z, mirror == Mirror.FRONT_BACK ? pair.x : CentralTowerSegment.SIDE_LENGTH - pair.x);
			case SOUTH ->
					origin.offset(mirror == Mirror.LEFT_RIGHT ? pair.x : CentralTowerSegment.SIDE_LENGTH - pair.x, pair.z, -1);
			default ->
					origin.offset(mirror == Mirror.LEFT_RIGHT ? CentralTowerSegment.SIDE_LENGTH - pair.x : pair.x, pair.z, CentralTowerSegment.SIDE_LENGTH);
		};
	}
}
