...
public class MagicPaintingVariants {
	...
	public static void bootstrap(BootstrapContext<MagicPaintingVariant> context) {
		ResourceLocation back = TwilightForestMod.prefix("magic_paintings/back");
		register(context, DARKNESS, 4, 2, back, List.of(
		...
		register(context, LUCID_LANDS, 3, 3, back, List.of(
			new Layer("background", null, null, true, true),
		...
		register(context, THE_HOSTILE_PARADISE, 3, 2, back, List.of(
		...
		register(context, CASTAWAY_PARADISE, 2, 4, back, List.of(
		...
		register(context, MUSIC_IN_THE_MIRE, 4, 3, back, List.of(
	...
	private static void register(BootstrapContext<MagicPaintingVariant> context, ResourceKey<MagicPaintingVariant> key, int width, int height, ResourceLocation backSprite, List<Layer> layers) {
		Component author = Component.translatable(key.location().toLanguageKey("magic_painting", "author"));
		context.register(key, new MagicPaintingVariant(width * 16, height * 16, layers, author, backSprite));
	}
