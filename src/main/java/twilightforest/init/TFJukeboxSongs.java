...
public final class TFJukeboxSongs {
	...
	private static void register(BootstrapContext<JukeboxSong> context, ResourceKey<JukeboxSong> key, SoundEvent sound, float length, int output) {
		Holder<SoundEvent> soundHolder = BuiltInRegistries.SOUND_EVENT.wrapAsHolder(sound);
		context.register(key, new JukeboxSong(soundHolder, Component.translatable(Util.makeDescriptionId("jukebox_song", key.location())), length, output));
	}
}
