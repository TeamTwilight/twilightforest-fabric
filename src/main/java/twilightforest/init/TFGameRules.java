...
public final class TFGameRules {
	...
	public static final Supplier<GameRules.Key<GameRules.BooleanValue>> ENFORCED_PROGRESSION_RULE = register("tfEnforcedProgression",
		...
		GameRules.BooleanValue.create(true, (server, value) -> {
			EnforceProgressionStatusPacket packet = new EnforceProgressionStatusPacket(value.get());
			PlayerLookup.all(server).forEach(player -> {
				if (ServerPlayNetworking.canSend(player, EnforceProgressionStatusPacket.TYPE)) {
