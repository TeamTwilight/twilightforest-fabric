...
import carminite.datagen.LanguageProvider;
...
public abstract class TFLangProvider extends LanguageProvider {
	...
	public TFLangProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> registries) {
		super(output, TFCommon.ID, "en_us");
		this.output = output;
	...
	public <T> void addGameRule(GameRule<T> gameRule, String gameRuleName) {
		this.add(gameRule.getDescriptionId(), gameRuleName);
	}

	public <T> void addGameRuleDescription(GameRule<T> gameRule, String gameRuleDescription) {
		this.add(gameRule.getDescriptionId() + ".description", gameRuleDescription);
	}
	...
	public void addEntityAndEgg(EntityType<?> entity, String name) {
		this.addEntityType(entity, name);
		this.add("item.twilightforest." + BuiltInRegistries.ENTITY_TYPE.getKey(entity).getPath() + "_spawn_egg", name + " Spawn Egg");
	}
	...
	public CompletableFuture<?> run(CachedOutput cache) {
		...
		CompletableFuture<?> languageGen = this.registries.thenCompose(provider -> {
			...
			if (!this.data.isEmpty())
				return this.save(cache, this.output.getOutputFolder(PackOutput.Target.RESOURCE_PACK).resolve(TFCommon.ID).resolve("lang").resolve("en_us.json"));
			return null;
		});
		...
		futuresBuilder.add(DataProvider.saveStable(cache, upsideDownFile, this.output.getOutputFolder(PackOutput.Target.RESOURCE_PACK).resolve(TFCommon.ID).resolve("lang").resolve("en_ud.json")));
