...
public abstract class StalactiteProvider implements DataProvider {
	...
	public CompletableFuture<?> run(CachedOutput output) {
		...
		map.forEach((identifier, stalactite) -> {
			...
			futuresBuilder.add(DataProvider.saveStable(output, Stalactite.CODEC.encodeStart(JsonOps.INSTANCE, stalactite).resultOrPartial(TFCommon.LOGGER::error).orElseThrow(), path));
		});
		configs.forEach(hillConfig -> {
			...
			futuresBuilder.add(DataProvider.saveStable(output, SpeleothemVarietyConfig.CODEC.encodeStart(JsonOps.INSTANCE, hillConfig).resultOrPartial(TFCommon.LOGGER::error).orElseThrow(), hillPath));
		});
